...
/*
 * Plugin Name: JP Gravatar Hovercards
...
function grofiles_hovercards_init() {
	...
	// Jetpack::enable_module_configurable( __FILE__ ); // E-1
	// Jetpack::module_configuration_load( __FILE__, 'gravatar_hovercards_configuration_load' ); // E-1
}
...
// add_action( 'jetpack_modules_loaded', 'grofiles_hovercards_init' ); // E-1
add_action( 'init', 'grofiles_hovercards_init' ); // E-1
...
function jetpack_grofiles_settings_link($actions) {
	return array_merge(
		array( 'settings' => sprintf( '<a href="%s">%s</a>', 'options-discussion.php#show_avatars', __( 'Settings', 'jetpack' ) ) ),
		$actions
	);
	return $actions;
}
